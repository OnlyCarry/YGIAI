...
#include "effect.h"
#include "group.h"
#include <pybind11/embed.h> // Para conectarte con Python
namespace py = pybind11;

// Indicador de IA personalizada
bool customAIEnabled = false;
...
void duel::set_response(const void* resp, size_t len) {
    ...
    // Interceptar decisiones del jugador IA personalizada
    if (customAIEnabled && isAIPlayer(game_field->player)) {
        auto action = getAIAction(game_field); // Método para obtener acción de la IA
        executeAIAction(action);              // Ejecutar acción de la IA
    }
}
int duel::getAIAction(field* game_field) {
    // Extraer el estado actual del juego
    GameState gameState = extractGameState(game_field);

    // Llamar a tu modelo de IA en Python
    py::scoped_interpreter guard{}; // Inicializa el intérprete de Python
    py::module ai_module = py::module::import("tu_modulo_ai");
    py::object action = ai_module.attr("decidir_accion")(gameState);

    return action.cast<int>();
}

void duel::executeAIAction(int action) {
    switch (action) {
        case 0:
            game_field->summon_monster(); // Ejemplo de función para invocar un monstruo
            break;
        case 1:
            game_field->activate_spell_trap(); // Activar mágica o trampa
            break;
        case 2:
            game_field->attack_with_monster(); // Atacar con un monstruo
            break;
        case 3:
            game_field->pass_turn(); // Pasar turno
            break;
        default:
            break;
    }
...
GameState duel::extractGameState(field* game_field) {
    GameState state;

    // Extraer información relevante del campo
    state.life_points[0] = game_field->player[0].lp;
    state.life_points[1] = game_field->player[1].lp;

    // Agrega más datos según sea necesario
    // Ejemplo: cartas en mano, en el campo, en el cementerio, etc.
